...
static void PrintMainMenu(PBOOT_CFG pBootCfg)
{
    ...
    EdbgOutputDebugString ( "3) Program CS8900 MAC address : [%X:%X:%X:%X:%X:%X]\r\n",
                           pBootCfg->EdbgAddr.wMAC[0] & 0x00FF, pBootCfg->EdbgAddr.wMAC[0] >> 8,
                           pBootCfg->EdbgAddr.wMAC[1] & 0x00FF, pBootCfg->EdbgAddr.wMAC[1] >> 8,
                           pBootCfg->EdbgAddr.wMAC[2] & 0x00FF, pBootCfg->EdbgAddr.wMAC[2] >> 8);
    ...
    EdbgOutputDebugString ( "7) Boot delay: %d seconds\r\n", pBootCfg->BootDelay);
    ...
    EdbgOutputDebugString ( "T) Download Target: [%s]\r\n", (pBootCfg->ConfigFlags & TARGET_TYPE_NAND) ? "Write to SDMMC Storage" : "*Download to RAM");
...
//    InitializeInterrupt();
    ...
    // Try to initialize the HSMMC controler and SD/MMC media.
    OALMSG(OAL_INFO, (TEXT("HSMMC_Init\r\n")));
    if ( !(g_bBootMediaExist = SDHC_INIT()) )
    {
        OALMSG(OAL_WARN, (TEXT("WARNING: OEMPlatformInit failed to initialize HSMMC as Boot Media.\r\n")));
    }
    else
    {
        ...
        RegInfo.MemBase.Reg[0] = (DWORD)OALPAtoVA(S3C6410_BASE_REG_PA_HSMMC1, FALSE);
