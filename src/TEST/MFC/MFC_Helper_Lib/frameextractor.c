...
static int next_delimiter(FRAMEX_CTX *pCTX, FILE *fpin, unsigned char *outbuf, const int outbuf_size, int *n_fill)
{
    ...
    if (outbuf != NULL) {
        if (pCTX->cont_offset == 0) {
            if((nbytes_to_write >=0) && (pCTX->delim_leng >=0) && (nbytes_to_write <= pCTX->delim_leng))
            {
...
static int next_frame_peek(FRAMEX_CTX *pCTX, FILE *fpin, unsigned char *peekbuf, const int peek_size, int *n_fill)
{
    ...
    if (peekbuf != NULL) {
        if (pCTX->cont_offset == 0) {
            if((nbytes_to_peek >=0) && (pCTX->delim_leng >=0) && (nbytes_to_peek <= pCTX->delim_leng))
            {
                return FRAMEX_ERR_BUFSIZE_TOO_SMALL;
            }
...
// 다음의 delimiter를 찾는다.
static int next_delimiter_mem(FRAMEX_CTX *pCTX, MMAP_STRM_PTR *strm_ptr, unsigned char *outbuf, const int outbuf_size, int *n_fill)
{
    int r;
    int l;
    int nbytes_to_write;
...
// 다음의 frame에서 원하는 수의 octets 값을 확인만 한다.
static int next_frame_peek_mem(FRAMEX_CTX *pCTX, MMAP_STRM_PTR *strm_ptr, unsigned char *peekbuf, const int peek_size, int *n_fill)
{
    int r;
    int l;
    int nbytes_to_peek;
...
int FrameExtractorFirst(FRAMEX_CTX *pCTX, FRAMEX_IN in)
{
    ...
    if (pCTX->in_type == FRAMEX_IN_TYPE_FILE)
        ...
        return next_delimiter_mem(pCTX, (MMAP_STRM_PTR *) in, NULL, 0, NULL);
}
...
int FrameExtractorNext(FRAMEX_CTX *pCTX, FRAMEX_IN in, unsigned char outbuf[], int outbuf_size, int *n_fill)
{
    ...
    if (pCTX->in_type == FRAMEX_IN_TYPE_FILE)
        ...
        return next_delimiter_mem(pCTX, (MMAP_STRM_PTR *) in, outbuf, outbuf_size, n_fill);
}
...
int FrameExtractorPeek(FRAMEX_CTX *pCTX, FRAMEX_IN in, unsigned char peekbuf[], int peek_size, int *n_fill)
{
    ...
    if (pCTX->in_type == FRAMEX_IN_TYPE_FILE)
        ...
        return next_frame_peek_mem(pCTX, (MMAP_STRM_PTR *) in, peekbuf, peek_size, n_fill);
}
/*
int FrameExtractorPeek(FRAMEX_CTX *pCTX, FRAMEX_IN in, unsigned char peekbuf[], int peek_size, int *n_fill)
{
