...
static void LogMsg()
{
    ...
    fopen_s(&fp,"\\NandFlash\\logmsg.txt", "at");
    if (fp == NULL)
        return;
    fprintf(fp, msg);
    fclose(fp);
...
//RETAILMSG(1,(L"\t ------------- \n"));
    ...
    for (pip=0; pip < dec_para->num_pips; pip++) {
        ...
        switch (dec_para->codec_mode[pip]) {
            ...
            case CODEC_H263:
            case CODEC_MPEG4:
                pFP_LIST[pip]->fp_SsbSipDecodeGetConfig(handle[pip], MPEG4_DEC_GETCONF_STREAMINFO, &(stream_info[pip]));
                break;  
            default:
                break;
        }
    ...
    while( !eof_flag ) {
        ...
        if (nLoop == perf_disp_period) {
            ...
            nLoop = 0;
            t1 = GetTickCount();
#if ( (PROPERTY_WINDOW == 1) && (!NO_OUTPUT) )
            if( dec_para->outDISP ) {
