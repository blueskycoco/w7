...
#define STREAMAPI_BASEID 1000
#define PERFORMANCE_BASEID 1100
#define STREAMAPIID(x) STREAMAPI_BASEID + x
#define PERFORMANCEID(x) PERFORMANCE_BASEID + x
...
BEGIN_FTE
    ...
    FTE(1,      "Test Stream Interface Open and Close work correctly",          STREAMAPIID(1),   0, SPITUX_OpenCloseVerify)
    FTE(1,      "Test Stream Interface IOCTL SPI_IOCTL_SET_CONFIG",             STREAMAPIID(2),   0, SPITUX_IoctlVerify_CmdConfig)
    FTE(1,      "Test Stream Interface IOCTL SPI_IOCTL_START",                  STREAMAPIID(3),   0, SPITUX_IoctlVerify_CmdStart)
    FTE(1,      "Test Stream Interface Read/Write data accuracy",               STREAMAPIID(4),   0, SPITUX_TransferData)
    FTH(0, "Stream Interface API Invalid parameters test cases")
    FTE(1,      "Test invalid parameters handle of SPI Read",                   STREAMAPIID(5),   0, SPITUX_InvalidRead)
    FTE(1,      "Test invalid parameters handle of SPI Write",                  STREAMAPIID(6),   0, SPITUX_InvalidWrite)
    FTE(1,      "Test invalid parameters handle of IOCTL SPI_IOCTL_SET_CONFIG", STREAMAPIID(7),   0, SPITUX_InvalidIOCTL_Config)
    FTH(0, "Transfer performance test cases")
    FTE(1,      "Transfer peroformance of SPI Read",                            PERFORMANCEID(1), 0, SPITUX_ReadPerformance)
    FTE(1,      "Transfer peroformance of SPI Write",                           PERFORMANCEID(2), 0, SPITUX_WritePerformance)
END_FTE
