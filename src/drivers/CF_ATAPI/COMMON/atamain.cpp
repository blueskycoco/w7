    ...
    )
{
    ...
    if (pDisk) {
        ...
        // Set the disk to IDLE state, ready to receive commands.
        if(!pDisk->SendDiskPowerCommand(ATA_NEW_CMD_IDLE, 0)) {
    ...
    )
{
    ...
    while (
        ...
        if (!pBus->m_szDevice[dwDeviceId]) {
            ...
            DWORD dwSize = wcslen(szDevKey) + wcslen(szNewKey) + 10;
            ...
            pBus->m_szDevice[dwDeviceId] = new TCHAR[dwSize];
            wcscpy_s(pBus->m_szDevice[dwDeviceId],dwSize,szDevKey);
            wcscat_s(pBus->m_szDevice[dwDeviceId],dwSize, L"\\");
            wcscat_s(pBus->m_szDevice[dwDeviceId],dwSize, szNewKey);
