...
{
    wcsncpy_s(m_szActiveKey,_countof(m_szActiveKey), szActiveKey, _TRUNCATE );
    m_szActiveKey[MAX_PATH - 1] = 0;
}

void CDisk::SetDeviceKey(TCHAR *szDeviceKey)
{
    wcsncpy_s(m_szDeviceKey,_countof(m_szDeviceKey), szDeviceKey, _TRUNCATE );
    ...
    )
{
    ...
    wcscpy_s(szProfile,_countof( psdi->szProfile), L"Default");
    ...
    if (IsAtapiDevice() && IsCDRomDevice()) {
        ...
        if (!hKey || !AtaGetRegistryString(hKey, REG_VALUE_CDPROFILE, &szProfile, sizeof(psdi->szProfile))) {
            wcscpy_s(psdi->szProfile,_countof(psdi->szProfile), REG_VALUE_CDPROFILE);
        }
        ...
        if (!hKey || !AtaGetRegistryString(hKey, REG_VALUE_6410_CFPROFILE, &szProfile, sizeof(psdi->szProfile))) {
            wcscpy_s(psdi->szProfile,_countof(psdi->szProfile), REG_VALUE_6410_CFPROFILE);
        }
    ...
    )
{
    ...
    *(pIOReq->pBytesReturned) = 0;
    ...
    wcscpy_s((PTCHAR) pIOReq->pOutBuf,pIOReq->dwOutBufSize, szDiskName);
