...
BOOL CSDHControllerCh1::Init(LPCTSTR pszActiveKey) {
    DEBUGMSG(SDCARD_ZONE_INIT,(TEXT("[HSMMC1] Initializing the HSMMC Host Controller\n")));
...
VOID CSDHControllerCh1::PowerUp() {
    DEBUGMSG(SDCARD_ZONE_INIT,(TEXT("[HSMMC1] Power Up the HSMMC Host Controller\n")));
...
BOOL CSDHControllerCh1::InitClkPwr() {
    ...
    if (pCLKPWR == NULL) {
        DEBUGMSG(SDCARD_ZONE_ERROR, (TEXT("[HSMMC1] Clock & Power Management Special Register is *NOT* mapped.\n")));
        return FALSE;
    }

#ifdef _SRCCLK_48MHZ_
    DEBUGMSG(SDCARD_ZONE_INFO, (TEXT("[HSMMC1] Setting registers for the USB48MHz (EXTCLK for SDCLK) : SYSCon.\n")));
...
#else
    DEBUGMSG(SDCARD_ZONE_INFO, (TEXT("[HSMMC1] Setting registers for the EPLL (for SDCLK) : SYSCon.\n")));
...
BOOL CSDHControllerCh1::InitGPIO() {
    ...
    if (pIOPreg == NULL) {
        DEBUGMSG(SDCARD_ZONE_ERROR, (TEXT("[HSMMC1] GPIO registers is *NOT* mapped.\n")));
        return FALSE;
    }

    // HSMMC Channel 1 supports 8-bit DAT
...
BOOL CSDHControllerCh1::InitHSMMC() {
    ...
    if (pHSMMC == NULL) {
        DEBUGMSG(SDCARD_ZONE_ERROR, (TEXT("[HSMMC1] HSMMC Special Register is *NOT* mapped.\n")));
        return FALSE;
    }

#ifdef _SRCCLK_48MHZ_
    DEBUGMSG(SDCARD_ZONE_INFO, (TEXT("[HSMMC1] Setting registers for the USB48MHz (EXTCLK) : HSMMCCon.\n")));
...
#else
    DEBUGMSG(SDCARD_ZONE_INFO, (TEXT("[HSMMC1] Setting registers for the EPLL : HSMMCCon.\n")));
    // Set the clock source to EPLL out for CLKMMC1
    pHSMMC->CONTROL2 = (pHSMMC->CONTROL2 & ~(0xffffffff)) |
        (0x3<<9) |  // Debounce Filter Count 0x3=64 iSDCLK
        (0x1<<8) |  // SDCLK Hold Enable
