                             ...
                             )
{
    ...
    DEBUGCHK(sdOption < _countof(sc_rgpszOptions));
    ...
    if (isri.dwSysintr == SYSINTR_NOP) {
        DEBUGMSG(SDCARD_ZONE_ERROR, (_T("%s No sysintr specified in registry\r\n"),
            pszFname));
                ...
                {
                    DEBUGMSG(SDCARD_ZONE_ERROR, (TEXT("%s IOCTL_HAL_REQUEST_SYSINTR HSMMC1 Failed \n\r"), pszFname));
                    goto EXIT;
                }
    }
    ...
    )
{
    ...
    if (hkDevice) {
        ...
        if (regDevice.ValueSZ(DEVLOAD_DLLNAME_VALNAME, szDll, _countof(szDll))) {
            szDll[_countof(szDll) - 1] = 0; // Null-terminate
            
            TCHAR szProc[MAX_PATH];

            if (regDevice.ValueSZ(SDHC_CREATION_PROC_KEY, szProc, _countof(szProc))) {
                szProc[_countof(szProc) - 1] = 0; // Null-terminate
