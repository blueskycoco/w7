        ...
        )
{
    ...
    DWORD dwCap = SD_SLOT_SD_4BIT_CAPABLE | 
        ...
        SD_SLOT_MMC_8BIT_CAPABLE |  // Channel 1 supports 8 bit
        ...
        )
{
    ...
    switch (sdOption) {
        ...
        case SDHCDSetSlotInterface: {
                                        ...
                                        if (sdCardInterfaceEx.InterfaceModeEx.bit.mmc8Bit != 0) {
        ...
        case SDHCDAckSDIOInterrupt:
                                      AckSDIOInterrupt();
                                      __fallthrough; // intentional fall through to enable interrupts after ACK'ing them
        ...
        )
{
    ...
    if (0 != pInterface->InterfaceModeEx.bit.mmc8Bit) {
        ...
        )
{
    ...
    if (pRequest) {
        DumpRequest(pRequest, SDCARD_ZONE_ERROR);
          ...
          DEBUGMSG(SDCARD_ZONE_ERROR,(TEXT("[HSMMC1] HandleErrors - ERR CMD:%d : "), pRequest->CommandCode));
    }
    ...
    if (wErrorStatus) {
        if ( wErrorStatus & ERR_INT_STATUS_CMD_TIMEOUT ) {
            ...
            switch(pRequest->CommandCode) {
                case 1 : DEBUGMSG(SDCARD_ZONE_ERROR, (TEXT("If the card is not a MMC, CMD 1 does not work.\n")));
                    break;
                case 5 : DEBUGMSG(SDCARD_ZONE_ERROR, (TEXT("If the card is not a SDIO, CMD 5 does not work.\n")));
                    break;
                case 8 : DEBUGMSG(SDCARD_ZONE_ERROR, (TEXT("If the card is not SD SPEC 2.0, CMD 8 does not work.\n")));
                    break;
                default :
                    DEBUGMSG(SDCARD_ZONE_ERROR, (TEXT("[HSMMC1] HandleErrors - CMD Timeout Error...\n")));
                    break;
            }
        }

        if ( wErrorStatus & ERR_INT_STATUS_CMD_CRC ) {
            DEBUGMSG(SDCARD_ZONE_ERROR,(TEXT("[HSMMC1] HandleErrors - CMD CRC Error...\r\n")));            
        ...
        if ( wErrorStatus & ERR_INT_STATUS_CMD_ENDBIT ) {
            DEBUGMSG(SDCARD_ZONE_ERROR,(TEXT("[HSMMC1] HandleErrors - CMD ENDBIT Error...\r\n")));                        
            status = SD_API_STATUS_RESPONSE_TIMEOUT;
        }

        if ( wErrorStatus & ERR_INT_STATUS_CMD_INDEX ) {
            DEBUGMSG(SDCARD_ZONE_ERROR,(TEXT("[HSMMC1] HandleErrors - CMD INDEX Error...\r\n")));            
            status = SD_API_STATUS_DEVICE_RESPONSE_ERROR;
        }

        if ( wErrorStatus & ERR_INT_STATUS_DAT_TIMEOUT ) {
            DEBUGMSG(SDCARD_ZONE_ERROR,(TEXT("[HSMMC1] HandleErrors - DAT Timeout Error...\r\n")));            
            status = SD_API_STATUS_DATA_TIMEOUT;
        }

        if ( wErrorStatus & ERR_INT_STATUS_DAT_CRC ) {
            DEBUGMSG(SDCARD_ZONE_ERROR,(TEXT("[HSMMC1] HandleErrors - Data CRC Error...\r\n")));              
            status = SD_API_STATUS_CRC_ERROR;
        }

        if ( wErrorStatus & ERR_INT_STATUS_DAT_ENDBIT ) {
            DEBUGMSG(SDCARD_ZONE_ERROR,(TEXT("[HSMMC1] HandleErrors - DAT END BIT Error...\r\n")));            
            status = SD_API_STATUS_DEVICE_RESPONSE_ERROR;
        }

        if ( wErrorStatus & ERR_INT_STATUS_BUS_POWER ) {
            DEBUGMSG(SDCARD_ZONE_ERROR,(TEXT("[HSMMC1] HandleErrors - Bus Power Error...\r\n")));
            status = SD_API_STATUS_DEVICE_RESPONSE_ERROR;
        }

        if ( wErrorStatus & ERR_INT_STATUS_AUTOCMD12 ) {
            DEBUGMSG(SDCARD_ZONE_ERROR,(TEXT("[HSMMC1] HandleErrors - AUTOCMD12 Error...\r\n")));            
            status = SD_API_STATUS_DEVICE_RESPONSE_ERROR;
        }

        if (wErrorStatus & ERR_INT_STATUS_ADMA) { // ADMA Error
            DEBUGMSG(SDCARD_ZONE_ERROR,(TEXT("[HSMMC1] HandleErrors - ADMA Error...\r\n")));
            if (m_SlotDma && pRequest ) {
        ...
        )
{
    ...
    if (dwClockRate > SD_FULL_SPEED_RATE)
    {
